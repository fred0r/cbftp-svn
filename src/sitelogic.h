...
std::list<DelayedCommand> delayedcommands;
  ...
  struct ExpiredCertDecision {
    int connid;
    std::string sitename;
    int expdays;
    bool decision_made;
    bool accept;
    ExpiredCertDecision() : connid(-1), expdays(0), decision_made(false), accept(false) {}
  };
  std::map<int, ExpiredCertDecision> pending_expired_certs;
  std::map<int, std::pair<std::string, int>> expired_cert_errors;
  ExpiredCertPromptCallback* expired_cert_prompt_callback;
...
void updateName();
  ...
  void setExpiredCertPromptCallback(ExpiredCertPromptCallback* cb);
  void expiredCertPrompt(int connid, const std::string& sitename, int expdays);
  void expiredCertError(int connid, const std::string& sitename, int expdays);
  void resumeWithExpiredCertDecision(int connid, bool accept);
  bool hasExpiredCertError(int requestid) const;
  std::string getExpiredCertErrorSite(int requestid) const;
  int getExpiredCertErrorDays(int requestid) const;
};
